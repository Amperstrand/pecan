#!/usr/bin/env bash
# Install/start/stop/status for the virtual charger (scripts/
...
EOF
  ssh "$SERVER" "systemctl daemon-reload && systemctl enable --now ev-virtual-charger"
  echo "installed + started (ev-virtual-charger.service)"
  ;;
start)
